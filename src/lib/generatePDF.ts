...
// Função para gerar PDF final com assinatura do técnico
export function generateFinalQuestionnairePDF(
  ...
  // Adicionar assinatura do técnico (NOVA LÓGICA)
  if (assinaturaTecnico && assinaturaTecnico.startsWith('data:image')) {
    try {
      const imgFormat = assinaturaTecnico.includes('image/png') ? 'PNG' : 'JPEG';
      doc.addImage(assinaturaTecnico, imgFormat, rightSignX + 3, yPos + 3, signatureWidth - 6, signatureHeight - 10);
    } catch (e) {
      doc.setFontSize(8);
      doc.setTextColor(...COLORS.textLight);
      doc.text("Assinatura não disponível", rightSignX + signatureWidth / 2, yPos + signatureHeight / 2, { align: "center" });
    }
  } else {
    doc.setFontSize(8);
    doc.setTextColor(...COLORS.textLight);
    doc.text("Assinatura não fornecida", rightSignX + signatureWidth / 2, yPos + signatureHeight / 2, { align: "center" });
  }
